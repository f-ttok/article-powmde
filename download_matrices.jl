...
for (url, matname) in zip(url_list, matname_list)
    ...
    # run(unpack_cmd("matrix/$(matname).tar", "matrix", ".tar", nothing)) # Uncomment if you use Windows
    mv("matrix/$(matname)/$(matname).mtx", "matrix/$(matname).mtx", force=true)
    rm("matrix/$(matname)", force=true, recursive=true)
    # rm("matrix/$(matname).tar", force=true, recursive=true) # Uncomment if you use Windows
